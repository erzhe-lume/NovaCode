...
## 背景
ch05 做完系统提示工程化后，NovaCode 已能自主多轮想 → 调工具 → 看结果地干活。但**工具执行是完全裸奔的**：工具执行链路上没有任何闸门——写文件工具能写任意路径、改文件工具能改任意文件、命令执行能跑任意命令（含 `rm -rf /`）、读写类工具能触碰项目目录之外的任何位置。这在 ch02 背景里就被列为预留能力（「在没有工具调用、**权限**、记忆等高级能力之前」）。

ch06 给 NovaCode 装上**五层防御的权限系统**：危险命令黑名单硬拦截、路径沙箱限定操作范围、可配置规则做细粒度放行/拦截、多档权限模式覆盖信任等级、人在回路作为最后确认。五层串联、逐层短路，一次工具调用在真正执行前必须穿过这道护栏。权限被拒绝时把结构化拒绝**回灌**给模型让它调整策略，而非终止 Agent Loop。这是从「会自己干」到「在安全护栏内自己干」的关键一跃。
...
- F8: 人在回路交互
  ...
  永久放行生成的规则采用**精确匹配**（针对该次具体调用的命令串或路径），不自动泛化。等待用户决策期间可取消（Esc / Ctrl+C），取消后会话历史仍合法、可继续对话。
...
- N8: 代码规范——按项目现有代码规范验收（本项目为 Java 21 + `-Xlint:all`）：`mvn compile` 无明显警告、`mvn clean package` 成功；遵循本项目既有代码风格（text block、record、无多余注释），不引入新工具链（ch05 决策）。
...
- AC16: 不破坏 ch04/ch05——多轮连环、用户取消、流出错恢复、历史一致、缓存命中、规划按轮次注入仍成立；临时无头集成验证通过、启动冒烟正常（项目暂无单测基础设施，验证沿用既有临时验证模式）。（N3）
- AC17: 代码规范与不泄漏——`mvn compile` 无明显警告、`mvn clean package` 成功；含密钥的本地配置层已被 gitignore，CLI 输出与配置回显不泄漏 api_key。（N8）
